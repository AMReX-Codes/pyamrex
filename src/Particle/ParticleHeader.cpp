...
void init_ParticleHeader(nb::module_& m)
{
    using namespace amrex;

    nb::class_<ParticleHeader>(m, "ParticleHeader")
        .def(nb::init<>())

        .def_static("read", &ParticleHeader::read,
            nb::arg("dir"), nb::arg("file"),
        ...
        .def_rw("version", &ParticleHeader::version,
            "raw version string of the file format")
        .def_rw("how", &ParticleHeader::how,
            "precision the data was written in: 'single' or 'double'")
        .def_rw("convert_ids", &ParticleHeader::convert_ids,
            "whether particle ids need conversion (Version_Two_Dot_One+)")
        .def_rw("dim", &ParticleHeader::dim,
            "AMREX_SPACEDIM the file was written with")
        .def_rw("num_real", &ParticleHeader::num_real,
            "number of real components (pure SoA: excludes the positions)")
        .def_rw("real_comp_names", &ParticleHeader::real_comp_names,
            "names of the real components (len == num_real)")
        .def_rw("num_int", &ParticleHeader::num_int,
            "number of integer components")
        .def_rw("int_comp_names", &ParticleHeader::int_comp_names,
            "names of the integer components (len == num_int)")
        .def_rw("is_checkpoint", &ParticleHeader::is_checkpoint,
            "True if the file is a checkpoint, False for a plotfile")
        .def_rw("num_particles", &ParticleHeader::num_particles,
            "total number of particles in the file")
        .def_rw("next_id", &ParticleHeader::next_id,
            "the next particle id to hand out")
        .def_rw("finest_level", &ParticleHeader::finest_level,
