...
def plot_mf(arr, compname, plo, phi):
    plt.plot()
    im = plt.imshow(arr.T, origin='lower', interpolation='none', extent=[plo[0], phi[0], plo[1], phi[1]], aspect='equal')
    plt.colorbar(im)
    plt.tight_layout()
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser("Plots each variable in a 2D plotfile using matplotlib.")
